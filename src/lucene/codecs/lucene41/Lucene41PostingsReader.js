...
var PostingsReaderBase = require('../PostingsReaderBase.js');
var IndexFileNames = require('library/lucene/index/IndexFileNames.js');
var Lucene41PostingsFormat = require('./Lucene41PostingsFormat.js');
...
var ForUtil = require('./ForUtil.js');

var BLOCK_SIZE = require('./Lucene41PostingsFormat.js').BLOCK_SIZE;
var MAX_DATA_SIZE = require('./ForUtil.js').MAX_DATA_SIZE;
var MAX_ENCODED_SIZE = require('./ForUtil.js').MAX_ENCODED_SIZE;

var ByteArrayDataInput = require('library/lucene/store/ByteArrayDataInput.js');
 ...
 * @lucene.experimental
 */
var Lucene41PostingsReader = defineClass({
	name : "Lucene41PostingsReader",
	extend : PostingsReaderBase,
	variables : {
		   docIn : null, //IndexInput
		   posIn : null, //IndexInput
		   payIn : null, //IndexInput
		   forUtil : null //ForUtil
	},
	...
	statics : {
		   ...
		   */
		   readVIntBlock : function(/* IndexInput */ docIn, /* int[] */ docBuffer, /* int[] */ freqBuffer, /* int */ num, /* boolean */ indexHasFreq){
		   		assert(docBuffer instanceof Array);
		   		assert(freqBuffer instanceof Array);
		   		
			    if (indexHasFreq) {
			    
			      for(var i=0;i<num;i++) {
			        var code = docIn.readVInt(); //int
			        docBuffer[i] = code >>> 1;
			        if ((code & 1) != 0) {
			          freqBuffer[i] = 1;
			        } else {
	...
	construct : function(/* Directory */ dir, /* FieldInfos */ fieldInfos, /* SegmentInfo */ segmentInfo, /* IOContext */ ioContext, /* String */ segmentSuffix){

		  // public static boolean DEBUG = false;
		
		  /** Sole constructor. */
		    var success = false;
		    var docIn = null; //IndexInput
		    var posIn = null; //IndexInput
		    var payIn = null; //IndexInput
	...
	methods : {
		  ...
		  //@Override
		   init : function(/* IndexInput */ termsIn){
		    ...
		    CodecUtil.checkHeader(termsIn,
		                          ...
		                          Lucene41PostingsWriter.VERSION_CURRENT,
		                          Lucene41PostingsWriter.VERSION_CURRENT);
		    var indexBlockSize = termsIn.readVInt();
		  ...
		  //@Override
		   readTermsBlock : function(/* IndexInput */ termsIn, /* FieldInfo */ fieldInfo, /* BlockTermState */ _termState)  {
		    var termState = /* (IntBlockTermState) */ _termState; //IntBlockTermState
		
		    var numBytes = termsIn.readVInt(); //int
		
		    if (termState.bytes == null) {
		      termState.bytes = new Array(ArrayUtil.oversize(numBytes, 1));  //new byte[ArrayUtil.oversize(numBytes, 1)];
		  ...
		  //@Override
		   nextTerm : function(/* FieldInfo */ fieldInfo, /* BlockTermState */ _termState){
...
module.exports = exports = Lucene41PostingsReader;
